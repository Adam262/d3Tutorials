...
var data = [];
// import data
// violates same origin policy in Chrome. Need to set up own server or disable Same Origin Policy via command line. 
// Disabling SOP is temporary, per session
d3.tsv('.././data/data.tsv', function(dataParam){
  if (dataParam === null) {
    return 'Data parameter cannot be null';
  } else {
    console.table(dataParam);
  }
  ...
  var toolTip = d3.select('#data-tsv-container')
                  .append('div')
                  .style({
                    'position': 'absolute',
                    'margin': '0 auto',
  ...
  var barChart = d3.select('#data-tsv-container')
    .append('svg')
      .attr({
        'width': width + margin.left + margin.right,
        'height': height + margin.top + margin.bottom 
      })
      .property('id', 'data-tsv')
      .append('g') // add svg element group
      .property('id', 'data-tsv-elements')
  ...
  var vGuide = d3.select('svg#data-tsv')
    .append('g')
    .property('id', 'vGuide')
  ...
  var hGuide = d3.select('svg#data-tsv')
    .append('g')
    .property('id', 'hGuide')
